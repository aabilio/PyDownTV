...
class TVAmurcia(object):
    '''
        Clase que maneja la descarga los vídeos de TV Autonómica de Murcia
    '''
    ...
    def procesarDescarga(self):
        ...
        if streamHTML.replace(" ", "").find("<h2>AlaCarta</h2>") != -1:
            printt(u"[INFO] A la Carta")
            streamer = streamHTML.split("&streamer=")[1].split("&")[0]
            file = streamHTML.split("&file=")[1].split("&")[0]
            url = streamer + "/" + file
            ext = "." + url.split(".")[-1]
            name = streamHTML.split("&title=")[1].split("&")[0] + ext
        else:
            ssalir(u"[!!!] No se reconoce el tipo de contenido")
